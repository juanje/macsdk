...
def cli(ctx: click.Context, version: bool) -> None:
    """DevOps Assistant - A DevOps chatbot with RAG for documentation and API tools"""
    if version:
        console.print("[bold cyan]DevOps Assistant[/] [dim]v0.1.0[/]")
        return
...
def _show_welcome() -> None:
    ...
    title = Text("DevOps Assistant", style="bold cyan")
    subtitle = Text(
        "DevOps chatbot with RAG for documentation and API tools", style="dim"
    )
    ...
    commands_table.add_row("tools", "List tools for agents")
    ...
    console.print("\n[dim]Examples:[/]")
    console.print("  [green]devops-chatbot chat[/]   Start CLI chat")
    console.print("  [green]devops-chatbot web[/]    Start web at localhost:8000")
    console.print("  [green]devops-chatbot --help[/] Show all options\n")


@cli.command(name="tools")
@click.argument("agent_name", required=False)
@click.option("--all", "-a", "show_all", is_flag=True, help="Show tools for all agents")
@click.pass_context
def tools_command(ctx: click.Context, agent_name: str | None, show_all: bool) -> None:
    """List tools for a specific agent or all agents.

    \b
    Examples:
      devops-chatbot tools              # Show available agents
      devops-chatbot tools api_agent    # Show tools for api_agent
      devops-chatbot tools --all        # Show tools for all agents
    """
    ...
    # Case 1: Show all agents with their tools
    if show_all:
        if not registered:
            console.print(
                Panel(
                    "[dim]No agents registered yet.\n\n"
                    "Add agents in [white]src/devops_chatbot/agents.py[/][/]",
                    title="[bold]🤖 Agents & Tools[/]",
                    border_style="yellow",
                )
            )
            return
        ...
        for agent_info in registered:
            ...
            console.print(f"[bold cyan]{name}[/] [dim]({len(agent_tools)} tools)[/]")
    ...
    # Case 2: Show tools for specific agent
    if agent_name is not None:
        if not registry.is_registered(agent_name):
            console.print(f"[red]✗ Error:[/] Agent '{agent_name}' not found")
            console.print(
                "[dim]Use[/] [green]devops-chatbot agents[/] "
                "[dim]to see registered agents.[/]\n"
            )
            return
        ...
        if not agent_tools:
            console.print(f"[yellow]Agent '{agent_name}' has no tools[/]\n")
            return

        table = Table(
            title=f"[bold]🔧 Tools for {agent_name}[/]",
            show_header=True,
            header_style="bold magenta",
            border_style="dim",
            title_justify="left",
        ...
        console.print(table)
        console.print(f"\n[dim]Total: {len(agent_tools)} tools[/]\n")
        return

    # Case 3: No argument - show agents with hint about tools command
    console.print(
    ...
    console.print(
        "[dim]To see tools for a specific agent:[/]\n"
        "  [green]devops-chatbot tools <agent_name>[/]\n"
        "\n[dim]To see all tools for all agents:[/]\n"
        "  [green]devops-chatbot tools --all[/]\n"
    )
...
def info() -> None:
    ...
    info_text = Text()
    info_text.append(
        "DevOps chatbot with RAG for documentation and API tools\n\n", style="dim"
    )
    ...
    try:
        ...
        info_text.append(f"{', '.join(agent_names)}\n", style="cyan")
    ...
    panel = Panel(
        info_text,
        title="[bold cyan]DevOps Assistant[/]",
        border_style="cyan",
        padding=(1, 2),
    )
    console.print(panel)
    console.print(
        "\n[dim]Use[/] [green]devops-chatbot agents[/] [dim]to see agent details.[/]\n"
    )
...
def index(force: bool) -> None:
    ...
    """
    try:
        # Lazy import heavy RAG dependencies
        ...
        if not config.enabled:
            console.print("\n[yellow]⚠ RAG is disabled in config.yml[/]\n")
            return

        # Show sources table
        console.print()
        table = Table(
            title=f"[bold]Sources to index ({len(config.sources)})[/]",
            show_header=True,
            header_style="bold",
            border_style="dim",
        )
        ...
        if force:
            console.print("\n[yellow]🔄 Force re-indexing enabled[/]")
            clear_rag_cache()

        console.print()
        with console.status("[cyan]Indexing documentation...[/]", spinner="dots"):
            retriever = create_retriever(config=config, force_reindex=force)

        if retriever:
            console.print("[green]✓ Indexing complete![/]")
            console.print("[dim]You can now use RAG queries in the chat.[/]\n")
        else:
